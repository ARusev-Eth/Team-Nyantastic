...
public class ControlsV3_Ground : MonoBehaviour 
{
	...
	//Public variables
	public GameObject[] groundCheck; //Stores information about position to check for ground
	...
	public bool grounded; //Stores information about player colliding with the ground
	...
	void FixedUpdate()
	{
		...
		if (Mathf.Abs (rigidbody2D.velocity.x) > maxVelocity)
			//...change the object's velocity to equal the maximum velocity
				rigidbody2D.velocity = new Vector2 (Mathf.Sign (rigidbody2D.velocity.x) * maxVelocity,
			                                     rigidbody2D.velocity.y);

		//If the left button has been clicked/touched and move direction has been set to a value
		if (moveDirection * rigidbody2D.velocity.x < maxVelocity)
			//...apply force in that direction
				rigidbody2D.AddForce (Vector2.right * moveDirection * moveForce);
